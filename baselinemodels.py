...
folder = 'C:/Users/camer/Documents/Python Scripts/Advising/TimeGPT/TimeGPT/'

GDP = pd.read_csv(folder + 'GDP.csv', sep = ',', header = 0, usecols = list(range(1,34)))
Cons = pd.read_csv(folder + 'Consumption.csv', sep = ',', header = 0, usecols = list(range(1,34)))
Un = pd.read_csv(folder + 'Unemployment.csv', sep = ',', header = 0, usecols = list(range(1,34)))
...
for i in range(6):
    ...
    for j in range(33):
        Train_x = pd.concat([GDP.iloc[:-3-i,j], Cons.iloc[:-3-i,j], Un.iloc[:-3-i,j]], axis = 1, ignore_index=True)
        Train_y = GDP.iloc[1+i:-2,j]
        Test_x = sm.add_constant(pd.concat([GDP.iloc[-3-i:-1-i,j], Cons.iloc[-3-i:-1-i,j], Un.iloc[-3-i:-1-i,j]],axis = 1, ignore_index=True), has_constant= 'add')
        Test_y = GDP.iloc[-2:,j]
        idx = Test_y[Test_y < -99].index
        Test_y = Test_y.drop(idx)
        Test_x = Test_x.drop(idx-1-i)
...
for i in range(6):
    ...
    for j in range(33):
        if j == 0:
            Train_x = sm.add_constant(pd.concat([GDP.iloc[:-3-i,j], Cons.iloc[:-3-i,j], Un.iloc[:-3-i,j]], axis = 1, ignore_index=True))
            Train_y = GDP.iloc[1+i:-2,j]
            Test_x = sm.add_constant(pd.concat([GDP.iloc[-3-i:-1-i,j], Cons.iloc[-3-i:-1-i,j], Un.iloc[-3-i:-1-i,j]],axis = 1, ignore_index=True), has_constant= 'add')
            Test_y = GDP.iloc[-2:,j]
            idx = Test_y[Test_y < -99].index
            ...
            Train_x_t = sm.add_constant(pd.concat([GDP.iloc[:-3-i,j], Cons.iloc[:-3-i,j], Un.iloc[:-3-i,j]], axis = 1, ignore_index=True))
            Train_y_t = GDP.iloc[1+i:-2,j]
            Test_x_t = sm.add_constant(pd.concat([GDP.iloc[-3-i:-1-i,j], Cons.iloc[-3-i:-1-i,j], Un.iloc[-3-i:-1-i,j]],axis = 1, ignore_index=True), has_constant= 'add')
            Test_y_t = GDP.iloc[-2:,j]
            idx = Test_y_t[Test_y_t < -99].index
            Test_y_t = Test_y_t.drop(idx)
            Test_x_t = Test_x_t.drop(idx-1-i)
